...
The \ce{PbWO4}-based detector was implemented as 32 \ce{PbWO4} sheets with the dimensions from Section~\ref{section:detector:concept}.
...
Here PTFE is modeled after its molecular formula of the repeating unit \ce{C2F4} with a density of $\rho_{\text{PFTE}}=\SI{2.2}{\gram\per\cm\cubed}$ and a mean exictiation energy of $I_{\text{PTFE}} = \SI{99.1}{\electronvolt}$  [citationneeded] and the aluminum foil (\SI{99.9}{\percent}) is approximated by solid aluminum. 
...
The PTFE-membrane is the Tetratex\textsuperscript{{\textregistered}}~ePTFE-membrane with \SI{127}{\micro\meter} membrane thickness at a density of \SI{0.35}{\gram\per\centi\meter^3}~\cite{rene}. % check density
...
\begin{figure}[h]
    \centering        
    \subfloat[Cleaned \ce{PbWO4} crystal.\label{fig:pwo:wrapping:cleaned}]
        {\includegraphics[width = 0.45\textwidth]{fig/pbwo4Detector/crystals/wrapping/crystal.jpg}}
    \hspace{0.02\textwidth}
    \subfloat[Fully wrapped \ce{PbWO4} crystal with window cut-out.\label{fig:pwo:wrapping:full}]
        {\includegraphics[width = 0.45\textwidth]{fig/pbwo4Detector/crystals/wrapping/wrapped_crystal.jpg}}\\
\end{figure}
